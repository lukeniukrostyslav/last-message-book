# EDITORIAL_BLOCK_01.md

# Editorial Block 01 — Opening & Promise

## Goal
Strengthen the opening contract of the novel: the warning, 30-day clock, future-looking photograph, and immediate uncertainty must feel like one escalating chain rather than separate devices.

## Canon constraints
- Alexei Morozov, 38.
- M is an automated protocol, not supernatural.
- The photograph is manufactured future-looking evidence.
- Day 30 is the deletion window.
- No reveal may depend on impossible time travel.

## Pass result
Status: **IN PROGRESS — opening chapters 1–9 revised**
Structural target: **100%**
Prose expansion: **75%**
Continuity verification: **85%**

## Required rewrite targets
1. Expand sensory detail around the first message.
2. Make Alexei's professional verification feel concrete.
3. Strengthen the transition from warning to photograph.
4. Seed the PR-06 signature without explaining it too early.
5. Preserve ambiguity around the sender.
6. Avoid explicitly revealing M's mechanism before its planned reveal.

## Quality gate
This block is not marked complete until the actual chapter text is revised and re-read against CANON_LOCK, MASTER_PLOT and TWISTS.
